...
public interface IGroupInvitationRepository {
   ...
   *     longer be valid
   * @return an {@link Optional} containing the created GroupInvitationDbo if the creation was successful,
   *     or an empty {@link Optional} if the creation failed (e.g., due to invalid input or database
   *     constraints)
   */
   ...
   * @return an {@link Optional} containing the found GroupInvitationDbo if a matching record is found, or
   *     an empty {@link Optional} if no matching record exists for the provided token
   */
