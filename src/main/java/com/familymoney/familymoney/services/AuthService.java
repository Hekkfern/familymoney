...
public class AuthService implements IAuthService {
    ...
    private EmailVerificationDbo generateAndStoreEmailVerificationToken(@NonNull UserId userId) {
        ...
        for (int attempt = 0; attempt < MAX_NUM_ATTEMPTS; attempt++) {
            ...
            log.warn("Token collision detected when generating email verification token for userId {}. Retrying...",
                    userId);
        }
    ...
    @Override
    public void registerUser(@NonNull Username username, @NonNull Email email, @NonNull Password password) {
        ...
        // Create user
        val userDbOpt = userRepository.create(username, email, passwordEncoder.encode(password.value()));
        ...
        emailSenderService.sendEmailVerificationEmail(email, username, emailVerificationTokenDb.token());
    ...
    public TokenPair loginUser(@NonNull Email email, @NonNull Password password) {
        ...
        val refreshTokenDbOpt = refreshTokenRepository.create(userDb.id(), refreshToken, UUID.randomUUID());
    ...
    public TokenPair refreshTokens(@NonNull RefreshToken refreshToken) {
        ...
        if (refreshTokenFoundInDb.isUsed()) {
            ...
            val userDbOpt =
                    userRepository.findById(refreshTokenFoundInDb.userId());
            if (userDbOpt.isEmpty()) {
        ...
        val refreshTokenCreatedInDbOpt = refreshTokenRepository.create(
                refreshTokenFoundInDb.userId(),
                newRefreshToken,
                refreshTokenFoundInDb.family()
        );
    ...
    public void verifyEmail(@NonNull EmailVerificationToken token) {
        ...
        val verificationTokenFromDbOpt =
    ...
    public void resendVerificationEmail(@NonNull Email email) {
        ...
        // Send verification email
        emailSenderService.sendEmailVerificationEmail(email, userFromDb.username(), emailVerificationTokenDb.token());
    }
