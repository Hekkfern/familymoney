...
public class UserController implements IUserController {
    ...
    public void updateMyUser(UpdateUserRequestDto request) {
        ...
        // Delete user
        userService.updateMyUser(userId,
                request.username() != null ? Optional.of(request.username()).map(Username::new) : Optional.empty(),
                request.email() != null ? Optional.of(request.email()).map(Email::new) : Optional.empty(),
                request.password() != null ? Optional.of(request.password()).map(Password::new) : Optional.empty()
        );
    }
}
